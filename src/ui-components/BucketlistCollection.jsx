...
export default function BucketlistCollection(props) {
    ...
    while ((newCache.length < cacheUntil || !isPaginated) && newNext != null) {
      ...
      const result = (
        await client.graphql({
          query: listStockBuckets.replaceAll("__typename", ""),
          variables,
        })
      ).data.listStockBuckets;
  ...
  return (
    <div>
      <Collection
        ...
        {...getOverrideProps(overrides, "BucketlistCollection")}
        {...rest}
      >
        {(item, index) => {
          if (loading) {
